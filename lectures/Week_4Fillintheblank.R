  ...
  library(tidyverse)
    ...
    surveys <- read_csv("data/portal_data_joined.csv")
      ...
      str(surveys)
        ...
        #select columns
        month_day_year <- select(surveys, month, day, year) #select is for selecting columns
        str(month_day_year)
          
          #filtering by equals
          year_1981 <- filter(surveys, year == 1981) #selecting is for selecting rows
            ...
            the80stidy <- filter(surveys,year %in% 1981:1983)
                   ...
                   the80srecycle
                         ...
                         #as its first element
                          ...
                          %>% filter(month==1)
                          #what happened here? 
                          # pipe needs to be on first line. Can split after pipe or open parenthesis
